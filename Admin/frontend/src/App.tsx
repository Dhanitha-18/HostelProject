...
function Sidebar() {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-[96px]' : 'w-[280px]'} m-4 mr-0 rounded-[2rem] bg-white/60 backdrop-blur-3xl border border-white/60 shadow-[10px_10px_40px_-10px_rgba(0,0,0,0.05),-1px_-1px_2px_rgba(255,255,255,0.5)] hidden md:flex flex-col h-[calc(100vh-2rem)] sticky top-4 transition-all duration-300 z-20 overflow-visible relative shrink-0`}>
      ...
      <div className={`flex flex-col items-center justify-center border-b border-slate-100/50 shrink-0 transition-all duration-300 ${isCollapsed ? 'p-6' : 'p-8'}`}>
        <div className="flex flex-col items-center justify-center w-full">
          <div className={`bg-indigo-600 rounded-[1rem] flex items-center justify-center shadow-lg shadow-indigo-600/30 transition-all duration-300 shrink-0 ${isCollapsed ? 'w-10 h-10 mb-0' : 'w-12 h-12 mb-4'}`}>
      ...
      <nav className={`space-y-2 mt-4 flex-1 overflow-y-auto overflow-x-hidden scrollbar-none ${isCollapsed ? 'px-3' : 'px-5'}`}>
        {navItems.map((item) => {
          ...
          return (
            <Link 
              ...
              className={`relative flex items-center py-3.5 mb-1 text-sm font-bold transition-all duration-300 rounded-2xl group outline-none hover:-translate-y-0.5 ${!isActive && 'hover:bg-white/50 hover:shadow-sm'} ${isCollapsed ? 'px-0 justify-center' : 'px-4'}`}
            >
              {isActive && (
                <motion.div 
                  layoutId="active-nav" 
      ...
      <div className={`mt-auto transition-all duration-300 ${isCollapsed ? 'p-3' : 'p-5'}`}>
         ...
         <div 
           ...
           title={isCollapsed ? "Switch Profile" : "Click to switch profile"}
         >
           ...
           <div className={`flex items-center justify-between overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 ml-0' : 'w-full opacity-100 ml-3'}`}>
             ...
             <div className="w-8 h-8 rounded-full bg-slate-100/50 flex items-center justify-center group-hover:bg-indigo-50 transition-colors ml-2 shrink-0">
               <LogOut className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 transition-colors translate-x-0 group-hover:translate-x-0.5" />
             </div>
           </div>
         </div>
      </div>
