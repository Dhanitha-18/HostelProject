...
export default function BlockOverview() {
  ...
  const blockFloors = activeBlock 
    ? Array.from(new Set(activeBlock.rooms.map((r: any) => r.floor))).sort() as number[]
    : [];
          ...
          >
            {blocks?.map((block: any) => {
              ...
              return (
                <div 
                  key={block.id}
                  onClick={() => { setSelectedBlock(block); setSelectedFloor(0); }}
                ...
                >
                  <div 
                    className="h-48 relative overflow-hidden bg-slate-100 group/image"
                  ...
                  >
                    ...
                    {/* Hover Drop Overlay */}
                    <div className="absolute inset-0 z-30 bg-black/40 opacity-0 group-hover/image:opacity-100 flex flex-col items-center justify-center text-white transition-opacity backdrop-blur-[2px] pointer-events-none hidden md:flex">
                      <Upload className="w-8 h-8 mb-2 drop-shadow-md" />
                      <span className="font-bold drop-shadow-md">Drop Photo to Update</span>
                    </div>
