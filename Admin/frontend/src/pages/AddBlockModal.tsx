...
export default function AddBlockModal({ isOpen, onClose }: AddBlockModalProps) {
  ...
  const [floorConfigs, setFloorConfigs] = useState(
    Array.from({ length: 3 }).map((_, i) => ({ id: Math.random().toString(36).substring(2, 15), floor: i, roomInput: `${i}01-${i}10`, capacity: 2 }))
  );
  ...
  const handleNumFloorsChange = (val: number) => {
    ...
    setFloorConfigs(prev => {
      ...
      const highestFloor = prev.length > 0 ? Math.max(...prev.map(c => c.floor)) : -1;
      
      let newConfigs = [...prev];
      
      // If we increased floors, add one default config for each new floor
      if (val - 1 > highestFloor) {
  ...
  const addConfigForFloor = (floorIndex: number) => {
    setFloorConfigs(prev => {
       const newConfig = { id: Math.random().toString(36).substring(2, 15), floor: floorIndex, roomInput: `${floorIndex}01`, capacity: 2 };
       const lastIndex = prev.map(c => c.floor).lastIndexOf(floorIndex);
       const copy = [...prev];
       if (lastIndex >= 0) {
          ...
          <div>
            ...
            <div className="space-y-4">
              {Array.from({ length: numFloors }).map((_, floorIndex) => {
                 const configsForFloor = floorConfigs.filter(c => c.floor === floorIndex);
                 
                 return (
                   <div key={floorIndex} className="border border-slate-200 rounded-xl p-4 bg-white shadow-sm transition-all hover:border-slate-300">
                     <div className="flex justify-between items-center mb-3">
                       <h4 className="font-extrabold text-slate-700 bg-slate-100 px-3 py-1 rounded-md">Floor {floorIndex}</h4>
                       <button 
                         onClick={() => addConfigForFloor(floorIndex)} 
