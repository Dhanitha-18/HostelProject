...
export default function PaymentDashboard() {
  ...
  const { data: payments, isLoading } = useQuery<Payment[]>({
    queryKey: ['payments'],
    queryFn: async () => { const r = await fetch(`${API}/payments`); if (!r.ok) throw new Error(); return r.json(); },
    refetchInterval: 5000,
  });

  const { data: stats } = useQuery<PaymentStats>({
    queryKey: ['payment-stats'],
    queryFn: async () => { const r = await fetch(`${API}/payments/stats`); if (!r.ok) throw new Error(); return r.json(); },
    refetchInterval: 5000,
  });
