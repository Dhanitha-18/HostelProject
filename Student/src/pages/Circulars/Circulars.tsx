            ...
            filteredNotices.map(notice => (
              ...
              >
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => {
                        ...
                        showToast('🔗 Direct Notice Link copied to clipboard!');
                      }}
                ...
                <div className="space-y-2">
                  <h4 
                    onClick={() => openNoticeViewer(notice)}
                ...
                {notice.attachments.length > 0 && (
                  <div className="bg-slate-50/80 border border-slate-200/80 p-4 rounded-xl space-y-2.5">
                    ...
                    <div className="flex flex-wrap gap-2.5">
                      {notice.attachments.map((att, idx) => (
                        <div 
                          key={idx}
                          onClick={() => openNoticeViewer(notice)}
                          className="bg-white border border-slate-200 px-3.5 py-2 rounded-xl text-xs font-bold text-slate-800 flex items-center gap-2 hover:border-primary hover:text-primary hover:shadow-xs transition-all cursor-pointer group/att"
                        >
                          {att.type === 'pdf' && <FileText className="w-4 h-4 text-red-500 shrink-0" />}
                          {att.type === 'image' && <ImageIcon className="w-4 h-4 text-blue-500 shrink-0" />}
                          {att.type === 'excel' && <FileSpreadsheet className="w-4 h-4 text-emerald-600 shrink-0" />}
                          {att.type === 'link' && <Link2 className="w-4 h-4 text-amber-500 shrink-0" />}
                          <span>{att.name}</span>
                ...
                {/* Reactions & Action Buttons Bar */}
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-100 text-xs">
                  
                  {/* Reaction Chips */}
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      onClick={() => handleReaction(notice.id, 'going')}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 border transition-all ${
                        notice.userReaction === 'going'
                          ? 'bg-blue-100 text-blue-800 border-blue-300 shadow-xs'
                          : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      <ThumbsUp className="w-3.5 h-3.5 text-blue-600" />
                      <span>Going ({notice.reactions.going})</span>
                    </button>

                    <button
                      onClick={() => handleReaction(notice.id, 'useful')}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 border transition-all ${
                        notice.userReaction === 'useful'
                          ? 'bg-red-100 text-red-800 border-red-300 shadow-xs'
                          : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      <Heart className="w-3.5 h-3.5 text-red-500" />
                      <span>Useful ({notice.reactions.useful})</span>
                    </button>

                    <button
                      onClick={() => handleReaction(notice.id, 'thanks')}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 border transition-all ${
                        notice.userReaction === 'thanks'
                          ? 'bg-emerald-100 text-emerald-800 border-emerald-300 shadow-xs'
                          : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      <Award className="w-3.5 h-3.5 text-emerald-600" />
                      <span>Thanks ({notice.reactions.thanks})</span>
                    </button>

                    <button
                      onClick={() => handleReaction(notice.id, 'clarification')}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 border transition-all ${
                        notice.userReaction === 'clarification'
                          ? 'bg-amber-100 text-amber-800 border-amber-300 shadow-xs'
                          : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      <HelpCircle className="w-3.5 h-3.5 text-amber-600" />
                      <span>Need Info ({notice.reactions.clarification})</span>
                    </button>
                  </div>

                  {/* View Document & PDF Buttons */}
                  <div className="flex items-center gap-2.5">
                    ...
                    <button
                      onClick={() => handlePrintPDF(notice)}
                      className="bg-primary hover:bg-primary-dark text-white text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-1.5 shadow-xs transition-colors"
                    >
                ...
                {/* Student Q&A / Warden Replies Section */}
                <div className="bg-slate-50/80 border border-slate-200/80 p-4 rounded-xl space-y-3 pt-3">
                  <span className="text-[11px] font-extrabold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                    <MessageSquare className="w-4 h-4 text-primary" />
                    Student Inquiries & Warden Replies ({notice.comments.length})
                  </span>
                  ...
                  {notice.comments.length > 0 ? (
                    <div className="space-y-2.5 max-h-48 overflow-y-auto pr-1">
                      {notice.comments.map(c => (
                        <div 
                          key={c.id} 
                          className={`p-3 rounded-xl text-xs space-y-1.5 shadow-2xs ${
                            c.role === 'Warden' 
                              ? 'bg-blue-50/90 border border-blue-200 text-blue-950 ml-4 border-l-4 border-l-blue-600' 
                              : 'bg-white border border-slate-200 text-slate-800'
                          }`}
                        >
                          <div className="flex justify-between items-center text-[10px]">
                            <span className="font-extrabold flex items-center gap-1.5">
                              {c.role === 'Warden' && <CheckCircle2 className="w-3.5 h-3.5 text-blue-600" />}
                              {c.author} <span className="text-slate-400 font-semibold">({c.role})</span>
                            </span>
                            <span className="text-slate-400 font-medium">{c.timestamp}</span>
                          </div>
                          <p className="font-medium text-xs leading-relaxed">{c.text}</p>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs text-slate-400 font-medium italic">No questions asked yet. Have a doubt regarding this notice?</p>
                  )}
                  ...
                  <div className="flex gap-2 pt-1.5">
                    <input
                      type="text"
                      placeholder="Ask a clarifying question to the Warden office..."
                      value={newCommentText[notice.id] || ''}
                      onChange={e => setNewCommentText({ ...newCommentText, [notice.id]: e.target.value })}
                      onKeyDown={e => e.key === 'Enter' && handleAddComment(notice.id)}
                      className="flex-1 bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-semibold outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                    />
                    <button
                      onClick={() => handleAddComment(notice.id)}
                      className="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors shadow-xs"
                    >
