...
export const PaymentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (data.application) {
        ...
        setStudent(prev => ({
          ...
          parentContact: app.fatherName ? `${app.fatherName}` : prev.parentContact,
        }));
  ...
  // Fetch status on login and poll every 15 seconds for real-time updates
  useEffect(() => {
    if (!studentUsn || !isLoggedIn) return;
